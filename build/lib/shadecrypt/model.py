...
"""
class shadecrypt:
  def __init__(self,file:str,write:bool=True,id:bool=True,backup:bool=False):
    self.file = file
    self.write = write
    self.backup = backup
    self.id = id
  ...
  def itPending(self, *args, **kwargs):
  ...
  def __create__db__instance(self):
    ...
    print(f'{hologram}{self.file} \n\x1b[1;32mDB instance created, you can safely begin making changes now\x1b[1;0m 🙂')
  ...
  def __compare__is__diff__(self,backup):
    if os.path.exists(backup):
      bkup =  open(backup, 'r')
      return True if str(bkup) != self.c_content else False
    else:
      return False
